...
const ExamResultsAnalysis = () => {
  ...
  const handleExamCodeChange = async (examCode) => {
    setSelectedExamCode(examCode);
    await fetchAnalysis(examCode || null);
  };
  ...
  useEffect(() => {
    if (!analysisData || !analysisData.attempts) {
      setFilteredAttempts([]);
      return;
    }

    const result = analysisData.attempts.filter(attempt => {
      // User ID search
  ...
  }, [analysisData, searchUserId, filterStatus, filterStartDate, filterEndDate]);
      ...
      <div className="bg-white shadow rounded-lg p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              onClick={async () => await fetchAnalysis(selectedExamCode || null)}
              disabled={loading}
            ...
            <div className="px-6 py-4 border-b border-gray-200">
              <div className="flex items-center justify-between">
                ...
                <div className="text-sm text-gray-500">
                  {filteredAttempts.length} of {analysisData.attempts.length} attempts shown
                </div>
              </div>
            </div>
            <div className="overflow-hidden">
              {filteredAttempts.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  {analysisData.attempts.length > 0 && filteredAttempts.length === 0 && (
                    <button
                      onClick={clearFilters}
          ...
          {filteredAttempts.length > 0 && (
            <div className="bg-white shadow rounded-lg p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {Array.from(new Set(
                  filteredAttempts
                    .map(attempt => attempt.examCode) // get examCodes
                    .filter(code => code) // remove undefined or falsy codes
                )).map((examCode) => (
                  <span
                    key={examCode}
